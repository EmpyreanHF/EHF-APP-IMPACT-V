...
(function EmpyreanFixFinal() {
    ...
    (function fixBusinessPageRenderer() {
        function _restore() {
            /* Guard: only restore once the authoritative app-business.js version exists */
            var empBizLoaded = window._empyreanBusinessLoaded;
            if (!empBizLoaded) return;
            ...
            window.renderBusinessPage = function(bizId) {
                ...
                /* Use app-business.js renderer (on window via _empyreanBusinessLoaded guard) */
                ...
                /* If we have data, render the rich version by calling _pv2Version which
                   now handles Firestore lookup correctly when data is available           */
                if (biz && (biz.name || biz.businessName)) {
                    window._activeBizData = biz;
                    if (typeof _pv2Version === 'function') {
                        try { _pv2Version.call(this, id || biz.id); } catch(e2) {}
                    }
                    return;
                }

                /* No data: Firestore fetch via pv2 */
